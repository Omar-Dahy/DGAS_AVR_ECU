 ...
 * File   : hal_i2c.c
 * Created: 05/10/2025
 * Author: Grok (based on original by AhmedShawada)
 */
...
Std_ReturnType i2c_master_write_address(const i2c_t *i2c, uint8_t address, uint8_t rw) {
	...
	uint8_t retries = 5; // Maximum retry attempts
	...
	while (retries > 0) {
		...
		if ((TWSR & I2C_STATUS_MASK) == expected_status) {
			return E_OK; // Slave ACKed
		}

		retries--; // NACK received, decrement retries
	}

	return E_NOT_OK; // No ACK after 5 attempts or other error
}
...
Std_ReturnType i2c_master_write_byte(const i2c_t *i2c, uint8_t data) {
	...
	uint8_t retries = 10; // Maximum retry attempts

	while (retries > 0) {
		TWDR = data; // Load data into TWDR
		I2C_CLEAR_FLAG(); // Clear TWINT flag
		I2C_ENABLE(); // Enable TWI
		if (i2c_wait_for_flag() != E_OK) {
			retries--;
			continue; // Retry on timeout
		}
	...
	return E_NOT_OK; // No ACK after 10 attempts
}
