...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define MOTOR_PWM_FIXED 500
#define ANGLE_MS_PER_DEGREE 40
#define ANGLE_MS_PER_DEGREE_2 4

#define MOTOR_DIR_GPIO_Port GPIOB
#define MOTOR_DIR_PIN1 GPIO_PIN_4        // IN1
#define MOTOR_DIR_PIN2 GPIO_PIN_5        // IN2
/* USER CODE END PD */
...
/* USER CODE BEGIN PV */
int8_t received_angle = -30;		// Variable to hold the angle received over I2C
volatile int8_t g_angle_flag = 0;
int16_t readValue;
char buffer[100];
/* USER CODE END PV */
...
void Execute_Angle_Sequence(void);
...
int main(void)
{
  ...
  /* USER CODE END 2 */
 // Execute_Angle_Sequence();
  /* Infinite loop */
  /* USER CODE BEGIN WHILE */
  while (1)
  {
	  int8_t sequence[] = {30, 0, -30, 0};
	     for (int i = 0; i < sizeof(sequence)/sizeof(sequence[0]); i++)
	     {
	         //Motor_Rotate_To_Angle(sequence[i]);
	         Execute_Angle_Sequence();
	         sprintf(buffer, "Now at: %d\r\n", sequence[i]);
...
{
    const int8_t ANGLE_LIMIT_MIN = -85;
    const int8_t ANGLE_LIMIT_MAX = 85;

    if (target_angle > ANGLE_LIMIT_MAX) target_angle = ANGLE_LIMIT_MAX;
    if (target_angle < ANGLE_LIMIT_MIN) target_angle = ANGLE_LIMIT_MIN;

    int8_t actual_angle = Get_Actual_Angle();
    int8_t error = target_angle - actual_angle;
    ...
    if (abs(final_error) <= 2) {
        HAL_UART_Transmit(&huart2, (uint8_t *)"reached target accurately.\r\n", 32, 100);
    } else {

        HAL_UART_Transmit(&huart2, (uint8_t *)"Slight offset remains. Correcting...\r\n", 40, 100);

        HAL_GPIO_WritePin(MOTOR_DIR_GPIO_Port, MOTOR_DIR_PIN1, (final_error < 0) ? GPIO_PIN_SET : GPIO_PIN_RESET);
        HAL_GPIO_WritePin(MOTOR_DIR_GPIO_Port, MOTOR_DIR_PIN2, (final_error > 0) ? GPIO_PIN_SET : GPIO_PIN_RESET);

        __HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_3, 700);
        HAL_Delay(abs(final_error) * 6);
        ...
        int8_t corrected_angle = Get_Actual_Angle();
        int8_t corrected_error = abs(target_angle - corrected_angle);

        if (corrected_error <= 2)
            HAL_UART_Transmit(&huart2, (uint8_t *)"Corrected accurately.\r\n", 26, 100);
        else
            HAL_UART_Transmit(&huart2, (uint8_t *)"Final correction failed.\r\n", 29, 100);
    }
}

void Execute_Angle_Sequence()
{
    int8_t sequence[] = {0, 30, 0, -30, 0, -30, 30};
    for (int i = 0; i < sizeof(sequence)/sizeof(sequence[0]); i++)
    {
        Motor_Rotate_To_Angle(sequence[i]);
        char msg[50];
...
void HAL_I2C_SlaveRxCpltCallback(I2C_HandleTypeDef *hi2c)
{
  ...
  char msg[50];
  sprintf(msg, "Moved motor to angle: %d\r\n", received_angle);
  HAL_UART_Transmit(&huart2, (uint8_t *)msg, strlen(msg), 100);
}

int8_t Get_Actual_Angle(void)
{
    const uint16_t pot_zero  = 1450;
    const uint16_t pot_right = 2600;
    const uint16_t pot_left  = 3;
