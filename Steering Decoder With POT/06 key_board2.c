...
void Motor_Rotate_To_Angle(int8_t target_angle)
{
    ...
    while (1)
    {
        ...
        uint16_t speed = (abs(error) > 20) ? 800 : (abs(error) > 10 ? 650 : 500);
        uint16_t duration = (abs(error) > 20) ? 9 : (abs(error) > 10 ? 7 : 5);

        __HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_3, speed);
        HAL_Delay(abs(error) * duration);
        ...
        sprintf(buffer, " Correcting... Δ=%d\r\n", error);
        HAL_UART_Transmit(&huart2, (uint8_t *)buffer, strlen(buffer), 100);

        attempt++;
        if (attempt > 15)
        {
            HAL_UART_Transmit(&huart2, (uint8_t *)" Max attempts reached.\r\n", 26, 100);
            break;
        }
    }
}
