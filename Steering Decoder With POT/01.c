...
/* USER CODE BEGIN PV */
int8_t received_angle = 0;		// Variable to hold the angle received over I2C
...
void Motor_Rotate_To_Angle(int8_t target_angle)
{
	static int8_t current_angle = 0;
	    ...
	    while (abs(error) > 2)
	    {
	        ...
	        if ((abs(error) < 5))
	            HAL_Delay(ANGLE_MS_PER_DEGREE_2);
	        else
	            HAL_Delay(ANGLE_MS_PER_DEGREE);
