	  ...
	  if (HAL_UART_Receive(&huart2, (uint8_t *)&rx_char, 1, HAL_MAX_DELAY) == HAL_OK)
	       {
	           ...
	           if (rx_char == '\n' || rx_char == '\r')
	           {
	               input_buffer[index_t] = '\0';  
	               int target_angle = atoi(input_buffer);  
	               ...
	               Motor_Rotate_To_Angle((int8_t)target_angle);

	               
	               index_t = 0;
	               memset(input_buffer, 0, sizeof(input_buffer));
	           }
	           else if (index_t < sizeof(input_buffer) - 1)
	           {
	               input_buffer[index_t++] = rx_char;  
	           }
	       }
    /* USER CODE END WHILE */
...
void Motor_Rotate_To_Angle(int8_t target_angle)
{
    ...
    while (abs(error) > 1)
    {
        ...
        //  move step by step
        int8_t next_target = current_angle + (STEP_ANGLE * direction);

        // Clamp to avoid overshooting
        if ((direction == 1 && next_target > target_angle) ||
            (direction == -1 && next_target < target_angle))
        {
            next_target = target_angle;
        }

        //  Control direction
        HAL_GPIO_WritePin(MOTOR_DIR_GPIO_Port, MOTOR_DIR_PIN1, (direction < 0) ? GPIO_PIN_SET : GPIO_PIN_RESET);
        HAL_GPIO_WritePin(MOTOR_DIR_GPIO_Port, MOTOR_DIR_PIN2, (direction > 0) ? GPIO_PIN_SET : GPIO_PIN_RESET);

        __HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_3, 600);
        HAL_Delay(8 * STEP_ANGLE);  
        ...
        sprintf(buffer, " Step → Now at: %d | Δ=%d\r\n", current_angle, error);
        HAL_UART_Transmit(&huart2, (uint8_t *)buffer, strlen(buffer), 100);
    }

    sprintf(buffer, "Final Position: %d\r\n\r\n", current_angle);
    HAL_UART_Transmit(&huart2, (uint8_t *)buffer, strlen(buffer), 100);
}
