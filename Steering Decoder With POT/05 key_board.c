...
void Motor_Rotate_To_Angle(int8_t target_angle)
{
    ...
    sprintf(buffer, "[INFO] Moving to angle: %d\r\n", target_angle);
    HAL_UART_Transmit(&huart2, (uint8_t *)buffer, strlen(buffer), 100);
    ...
    for (int attempt = 0; attempt < 5; attempt++)
    {
        ...
        int8_t final_error = abs(target_angle - final_angle);
        ...
        if (final_error <= 3) {
            ...
            HAL_UART_Transmit(&huart2, (uint8_t *)" Re-adjusting...\r\n", 23, 100);
        }
    }

    HAL_UART_Transmit(&huart2, (uint8_t *)" Failed to reach accurate angle.\r\n\r\n", 38, 100);
}
